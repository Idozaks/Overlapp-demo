...
const formattedUsers = usersData.map(user => {
  ...
  if (user.identityPreferences && user.identityPreferences.attributeImportance) {
    const attrImportance = user.identityPreferences.attributeImportance;
    attributeImportanceStr = `
    attributeImportance: {
      ${Object.entries(attrImportance).map(([key, value]) => `${key}: ${value}`).join(',\n      ')}
    },`;
  }
...
const updatedRoutesContent = routesContent.replace(syntheticUsersRegex, syntheticUsersArrayStr);

// Write the updated content back to routes.ts
fs.writeFileSync(routesFilePath, updatedRoutesContent, 'utf8');

console.log(`Successfully updated routes.ts with ${usersData.length} synthetic users.`);